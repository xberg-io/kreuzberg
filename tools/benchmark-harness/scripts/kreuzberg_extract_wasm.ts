  ...
  for await (const line of rl) {
    ...
    try {
      ...
      process.stderr.write(
        `[wasm:ERROR] extraction #${extractionCount} failed: ${error.message}\n`,
      );
...
async function main(): Promise<void> {
  ...
  if (args.length < 1) {
    console.error(
      "Usage: kreuzberg_extract_wasm.ts [--ocr|--no-ocr] <mode> <file_path> [additional_files...]",
    );
  ...
  process.stderr.write(
    `[wasm] initializing (timeout=${EXTRACTION_TIMEOUT_MS}ms, debug=${DEBUG})\n`,
  );
