...
fn get_script_path(script_name: &str) -> Result<PathBuf> {
    ...
    let script_path = PathBuf::from("kreuzberg/tools/benchmark-harness/scripts").join(script_name);
    if script_path.exists() {
        return Ok(script_path);
    }
    ...
    if let Ok(root) = workspace_root() {
        let script_path = root.join("kreuzberg/tools/benchmark-harness/scripts").join(script_name);
        if script_path.exists() {
            return Ok(script_path);
        }
    }
