...
char *resolve_workspace_root(void) {
    /* The test binary lives in kreuzberg/e2e/c/.  Go three levels up to the workspace. */
#if defined(_WIN32) || defined(__CYGWIN__)
    /* On Windows use GetModuleFileName – fall back to relative path. */
    char *root = (char *)malloc(4096);
    if (!root) { fputs("OOM\n", stderr); exit(1); }
    strcpy(root, ".." PATH_SEP ".." PATH_SEP "..");
    return root;
#else
...
#  endif
    ...
    /* Dirname three times to go from  .../kreuzberg/e2e/c/test_xxx  ->  workspace */
    char *slash = strrchr(exe_path, '/');
    if (slash) *slash = '\0'; /* strip binary name -> .../kreuzberg/e2e/c */
    slash = strrchr(exe_path, '/');
    if (slash) *slash = '\0'; /* strip "c"        -> .../kreuzberg/e2e   */
    slash = strrchr(exe_path, '/');
    if (slash) *slash = '\0'; /* strip "e2e"      -> .../kreuzberg       */
    slash = strrchr(exe_path, '/');
    if (slash) *slash = '\0'; /* strip "kreuzberg" -> workspace           */
