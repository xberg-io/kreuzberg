...
- **#962**: PDF text extracted one character per line on glyph-spaced documents — when a PDF positions each glyph via a separate `BT..ET` block with an absolute `Tm` y-coordinate differing from adjacent glyphs by more than pdfium's internal line-break threshold (~3.2 pt), pdfium emits `\r\n` between every glyph instead of coalescing them into one line. Microsoft Word triggers this pattern for broken-image placeholder text. The extraction path now detects the fragmentation signature (≥ 5 consecutive `\r\n`-separated lines of ≤ 3 visible chars) and rebuilds the page text from per-character position data: characters are sorted by x-coordinate (reading order), grouped by baseline proximity (4 pt tolerance), and spaces are inserted at word gaps. Both the fast-path and tracking-path extraction invoke the repair automatically.
