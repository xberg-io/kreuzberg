...
- **candle-ocr**: `PaddleOcrVlBackend` now reuses engines across calls via a process-wide pool keyed by `(task, device_kind)`. Eliminates the ~900 MB safetensors reload that previously fired on every request.
