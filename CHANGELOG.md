...
- **e2e/ruby**: Fixed embed_texts and embed_texts_async e2e test failures caused by alef v0.17.17 Ruby generator regression (empty array literals instead of fixture data). Manually corrected array arguments in embeddings_spec.rb (e.g., `['Hello world', 'Test']`) and embed_async_pending_spec.rb to match fixture intent. Fixed config structure in fixture and tests: `embed_texts_async_preset_switch` now uses `{ model: { type: 'preset', name: 'balanced' } }` (correct EmbeddingConfig structure) instead of `{ embedding_model: 'default' }`. Changed `embed_texts_batch` config from empty hash `{}` to `nil` to trigger Ruby binding default. Brings Ruby e2e from 85/88 PASS to 87/88 PASS (only tesseract backend env failure remains). (`e2e/ruby/spec/embeddings_spec.rb`, `e2e/ruby/spec/embed_async_pending_spec.rb`, `e2e/ruby/spec/embed_extra_spec.rb`, `fixtures/embed_async_pending/embed_texts_async_preset_switch.json`)
