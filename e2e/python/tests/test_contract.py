...
# alef:hash:f30fd3a7c9380ccc8653190267e6a86e4165f2e8ce25b190cbe88b09e373e919
...
async def test_api_batch_file_async() -> None:
    ...
    paths = None
    result = await batch_extract_file(items=paths)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
    assert any(v in result.content for v in ["May 5, 2023", "Mallori"])  # noqa: S101
...
def test_api_batch_file_sync() -> None:
    ...
    paths = None
    result = batch_extract_file_sync(items=paths)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
    assert any(v in result.content for v in ["May 5, 2023", "Mallori"])  # noqa: S101
...
def test_api_batch_file_with_timeout_sync() -> None:
    ...
    result = batch_extract_file_sync(items=paths, config=config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
def test_config_llm_embeddings() -> None:
    ...
    assert len(result.chunks) >= 1  # noqa: S101
    assert all(
        c.embedding is not None and len(c.embedding) > 0 for c in (result.chunks or [])
    )  # noqa: S101
    assert all(c.content for c in (result.chunks or []))  # noqa: S101
