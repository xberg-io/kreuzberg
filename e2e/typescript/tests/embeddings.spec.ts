...
describe("embeddings fixtures", () => {
  it("embedding_async", async () => {
    ...
    const config = buildConfig({"chunking":{"max_chars":500,"max_overlap":50,"embedding":{"model":{"type":"preset","name":"balanced"},"normalize":true}}});
  ...
  it("embedding_fast_preset", () => {
    ...
    const config = buildConfig({"chunking":{"max_chars":500,"max_overlap":50,"embedding":{"model":{"type":"preset","name":"fast"},"normalize":true}}});
    let result: ExtractionResult | null = null;
    try {
      result = extractFileSync(documentPath, null, config);
    } catch (error) {
