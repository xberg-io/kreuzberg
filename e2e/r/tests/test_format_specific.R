...
# alef:hash:95db9b1eee32f3c9aeb3195406778aadb0a9f227ab1b5c8d7784ad759a4f9a69
...
test_that("format_pptx: PPTX presentation extraction using extract_file_sync", {
  result <- jsonlite::fromJSON(extract_file_sync(path = .resolve_fixture("pptx/simple.pptx"), mime_type = "application/vnd.openxmlformats-officedocument.presentationml.presentation", config = JsExtractionConfig$default()), simplifyVector = FALSE)
  expect_true(TRUE)
})

test_that("format_xlsx: XLSX spreadsheet extraction using extract_file_sync", {
  result <- jsonlite::fromJSON(extract_file_sync(path = .resolve_fixture("xlsx/stanley_cups.xlsx"), mime_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", config = JsExtractionConfig$default()), simplifyVector = FALSE)
  expect_true(TRUE)
})
