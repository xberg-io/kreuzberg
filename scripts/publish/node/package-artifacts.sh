...
if [ ! -d crates/xberg-node/npm ]; then
...
aarch64-apple-darwin)
  ...
  node_file="xberg-node.darwin-arm64.node"
  pdfium_file="libpdfium.dylib"
  ;;
x86_64-apple-darwin)
  platform_dir="darwin-x64"
  node_file="xberg-node.darwin-x64.node"
  pdfium_file="libpdfium.dylib"
  ;;
x86_64-pc-windows-msvc)
  platform_dir="win32-x64-msvc"
  node_file="xberg-node.win32-x64-msvc.node"
  pdfium_file="pdfium.dll"
  ;;
aarch64-pc-windows-msvc)
  platform_dir="win32-arm64-msvc"
  node_file="xberg-node.win32-arm64-msvc.node"
  pdfium_file="pdfium.dll"
  ;;
x86_64-unknown-linux-gnu)
  platform_dir="linux-x64-gnu"
  node_file="xberg-node.linux-x64-gnu.node"
  pdfium_file="libpdfium.so"
  ;;
aarch64-unknown-linux-gnu)
  platform_dir="linux-arm64-gnu"
  node_file="xberg-node.linux-arm64-gnu.node"
  pdfium_file="libpdfium.so"
  ;;
armv7-unknown-linux-gnueabihf)
  platform_dir="linux-arm-gnueabihf"
  node_file="xberg-node.linux-arm-gnueabihf.node"
  pdfium_file="libpdfium.so"
  ;;
*)
...
dest="crates/xberg-node/npm/${platform_dir}/${node_file}"
src=""
...
for candidate in "crates/xberg-node/artifacts/${node_file}" "crates/xberg-node/${node_file}"; do
...
if [ -z "$src" ]; then
  ...
  echo "  - crates/xberg-node/artifacts/${node_file}" >&2
  echo "  - crates/xberg-node/${node_file}" >&2
  echo ""
  echo "Available .node files:" >&2
  find crates/xberg-node -maxdepth 3 -type f -name "*.node" -print 2>/dev/null || echo "  (none found)"
  echo ""
  echo "npm directory structure:" >&2
  find crates/xberg-node/npm -type d 2>/dev/null | head -20 || echo "  (npm directory not created)"
  echo ""
  echo "Full crates/xberg-node directory:" >&2
  find crates/xberg-node -type f \( -name "*.node" -o -name "package.json" \) | head -30
  exit 1
fi

platform_npm_dir="crates/xberg-node/npm/${platform_dir}"
echo ""
...
if [ -z "$pdfium_src" ]; then
  ...
  echo "  Expected locations:" >&2
  echo "    - crates/xberg-node/${pdfium_file}" >&2
  echo "    - target/release/${pdfium_file}" >&2
  echo "    - target/${target}/release/${pdfium_file}" >&2
else
  echo "  Copying ${pdfium_file} to platform directory..."
  cp -f "$pdfium_src" "crates/xberg-node/npm/${platform_dir}/${pdfium_file}"
  ls -lh "crates/xberg-node/npm/${platform_dir}/${pdfium_file}"

  platform_pkg_json="crates/xberg-node/npm/${platform_dir}/package.json"
  tmp_pkg_json="$(mktemp)"
...
tar -czf "node-bindings-${target}.tar.gz" -C crates/xberg-node/npm "${platform_dir}"
