...
// alef:hash:6930b118cb8582409bb8d98677481722255f6503603eabc9c67651cd7a30eb8c
...
#[allow(clippy::missing_errors_doc)]
#[pyfunction]
#[pyo3(signature = (path, mime_type, config))]
pub fn extract_file_sync_ffi(path: String, mime_type: String, config: ExtractionConfig) -> PyResult<ExtractionResult> {
    let config_json =
        serde_json::to_string(&config).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    let config_core: kreuzberg::ExtractionConfig =
        serde_json::from_str(&config_json).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    kreuzberg::extract_file_sync_ffi(&path, &mime_type, &config_core)
        .map(ExtractionResult::from)
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pub fn _kreuzberg(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(extract_file_sync_ffi, m)?)?;
    m.add_function(wrap_pyfunction!(extract_file_ffi, m)?)?;
