...
// alef:hash:932470ab1b4035c1527489bc860a5c4dc117244287cdd603d02a5fe4ebc11755
...
pub fn batch_extract_file_sync(items: Vec<BatchFileItem>, config: ExtractionConfig) -> PyResult<Vec<ExtractionResult>> {
    let items_json =
        serde_json::to_string(&items).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    let items_core: Vec<kreuzberg::BatchFileItem> =
        serde_json::from_str(&items_json).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    let config_json =
        serde_json::to_string(&config).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    let config_core: kreuzberg::ExtractionConfig =
        serde_json::from_str(&config_json).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
