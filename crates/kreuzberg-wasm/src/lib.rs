...
// alef:hash:3e665fd9f90e9fdf914b4d0eef9cd9db53f141bb94d6c2dcd0a4f2a881276e02
...
#[allow(clippy::missing_errors_doc)]
#[wasm_bindgen(js_name = "extractFileSyncFfi")]
pub fn extract_file_sync_ffi(
    ...
    let config_core: kreuzberg::ExtractionConfig =
        serde_wasm_bindgen::from_value::<kreuzberg::ExtractionConfig>(config)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    let result = kreuzberg::extract_file_sync_ffi(&path, &mime_type, &config_core)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result.into())
}
...
#[allow(clippy::missing_errors_doc)]
#[wasm_bindgen(js_name = "extractFileFfi")]
pub async fn extract_file_ffi(
    ...
    let config_core: kreuzberg::ExtractionConfig = config.into();
    let result = kreuzberg::extract_file_ffi(&path, &mime_type, &config_core)
        .await
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(WasmExtractionResult::from(result))
}
