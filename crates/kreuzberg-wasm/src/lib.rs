...
// alef:hash:1d892a44eeaa0ffa85a94efee9efe4841ece3005f363de61ab7de515568660b3
...
pub fn batch_reduce_tokens(
    ...
    let config_core: kreuzberg::TokenReductionConfig =
        serde_wasm_bindgen::from_value::<kreuzberg::TokenReductionConfig>(config)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    let result = kreuzberg::text::batch_reduce_tokens(&texts_refs, &config_core, language_hint.as_deref())
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result)
}
...
pub fn chunk_text(
    ...
    let page_boundaries_core: Option<Vec<kreuzberg::PageBoundary>> = page_boundaries
        ...
        .transpose()?;
    let result = kreuzberg::chunking::core::chunk_text(&text, &config_core, page_boundaries_core.as_deref())
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result.into())
}
...
pub fn chunk_texts_batch(texts: Vec<String>, config: JsValue) -> Result<Vec<WasmChunkingResult>, JsValue> {
    let config_core: kreuzberg::ChunkingConfig = serde_wasm_bindgen::from_value::<kreuzberg::ChunkingConfig>(config)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    let result = kreuzberg::chunking::core::chunk_texts_batch(&texts_refs, &config_core)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result.into_iter().map(Into::into).collect())
}
