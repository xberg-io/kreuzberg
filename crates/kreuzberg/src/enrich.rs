...
//!
//! ```ignore
//! use kreuzberg::{ExtractionConfig, extract_file, enrich, EnrichmentConfig};
//!
...
pub struct NerEnrichmentConfig {
    ...
    /// ```ignore
    /// use std::sync::Arc;
    /// use kreuzberg::{LlmBackend, LlmConfig, enrich::NerEnrichmentConfig};
    ///
...
pub struct EnrichmentConfig {
    ...
    /// `Err(KreuzbergError::Other("transcription backend not yet implemented"))`.
...
/// # Errors
///
/// - [`crate::KreuzbergError::Validation`] when the classification config has an
///   empty label set (propagated from `classify_document`).
/// - [`crate::KreuzbergError::Other`] when the NER or captioning backends fail.
