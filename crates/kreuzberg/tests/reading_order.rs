...
//! ```
//! cargo test -p kreuzberg --test reading_order plain_text
//! ```
//!
//! Run with layout detection (downloads layout ONNX model, ~300MB):
//! ```
//! KREUZBERG_RUN_LAYOUT_TESTS=1 cargo test -p kreuzberg --features full --test reading_order -- --nocapture
//! ```

#![cfg(all(feature = "pdf", feature = "layout-detection", not(target_arch = "wasm32")))]

use kreuzberg::core::config::{ExtractionConfig, OutputFormat, PdfConfig};
...
fn should_run_layout_tests() -> bool {
    std::env::var("KREUZBERG_RUN_LAYOUT_TESTS").is_ok()
}
...
fn text_reading_order_changes_output() {
    if !should_run_layout_tests() {
        eprintln!("Skipping layout-detection test (set KREUZBERG_RUN_LAYOUT_TESTS=1 to enable)");
        return;
    }
