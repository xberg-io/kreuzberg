...
async fn inline_preset_text_only_happy_path() {
    ...
    assert!(
        matches!(
            ...
            xberg::heuristics::StructuredCallMode::TextOnly
                | xberg::heuristics::StructuredCallMode::TextOnlyWithVisionFallback
        ),
...
async fn vision_fallback_fires_when_threshold_exceeded() {
    ...
    let options = StructuredOptions {
        ...
        force_call_mode: Some(xberg::heuristics::StructuredCallMode::TextOnlyWithVisionFallback),
        ..StructuredOptions::default()
    };
