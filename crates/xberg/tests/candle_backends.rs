...
async fn candle_hunyuan_ocr_e2e_extraction() {
    let model_path = match std::env::var("XBERG_HUNYUAN_OCR_MODEL_PATH") {
        Ok(p) => p,
        Err(_) => {
            println!("XBERG_HUNYUAN_OCR_MODEL_PATH not set — skipping Hunyuan-OCR e2e test");
            return;
        }
    };
...
async fn candle_deepseek_ocr_e2e_extraction() {
    let model_path = match std::env::var("XBERG_DEEPSEEK_OCR_MODEL_PATH") {
        Ok(p) => p,
        Err(_) => {
            println!("XBERG_DEEPSEEK_OCR_MODEL_PATH not set — skipping DeepSeek-OCR e2e test");
            return;
        }
    };
...
async fn candle_paddleocr_vl_e2e_extraction() {
    let model_path = match std::env::var("XBERG_PADDLEOCR_VL_MODEL_PATH") {
        Ok(p) => p,
        Err(_) => {
            println!("XBERG_PADDLEOCR_VL_MODEL_PATH not set — skipping PaddleOCR-VL e2e test");
            return;
        }
    };
