...
pub(crate) fn blake3_hash_file(path: &Path) -> Result<String> {
    let file = std::fs::File::open(path)
        .map_err(|e| crate::error::XbergError::cache(format!("Failed to open file for hashing: {e}")))?;
    ...
    loop {
        let n = reader
            .read(&mut buf)
            .map_err(|e| crate::error::XbergError::cache(format!("Failed to read file for hashing: {e}")))?;
        if n == 0 {
            break;
        }
        hasher.update(&buf[..n]);
...
pub(crate) fn get_available_disk_space(path: &str) -> Result<f64> {
    #[cfg(unix)]
    {
        ...
        let path_str = check_path
            .to_str()
            .ok_or_else(|| XbergError::validation("Path contains invalid UTF-8".to_string()))?;
        let c_path = CString::new(path_str).map_err(|e| XbergError::validation(format!("Invalid path: {}", e)))?;
