...
/// 2. `XBERG_PDFIUM_PATH` env var (directory **or** full path): bind from that location.
...
fn create_pdfium_bindings(lib_dir: &Option<PathBuf>) -> Result<Box<dyn PdfiumLibraryBindings>, String> {
    ...
    // XBERG_PDFIUM_PATH may be a directory or a full path to the library file.
    ...
    if let Ok(path_str) = std::env::var("XBERG_PDFIUM_PATH") {
