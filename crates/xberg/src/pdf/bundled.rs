...
//!    `XBERG_PDFIUM_BUNDLED_PATH` environment variable
//! 2. At compile time: `include_bytes!` embeds the library binary in the executable
//! 3. At runtime: `extract_bundled_pdfium()` extracts to `$TMPDIR/xberg-pdfium/`
...
//! # {
//! use xberg::pdf::bundled::extract_bundled_pdfium;
//!
...
fn bundled_library_info() -> (&'static str, &'static str) {
    if cfg!(target_os = "windows") {
        ("pdfium.dll", "xberg-pdfium")
    } else if cfg!(target_os = "macos") {
        ("libpdfium.dylib", "xberg-pdfium")
    } else {
        ("libpdfium.so", "xberg-pdfium")
    }
}
...
/// - Extracts to `$TMPDIR/xberg-pdfium/` (non-WASM only)
...
pub(crate) fn extract_bundled_pdfium() -> io::Result<PathBuf> {
    ...
    let bundled_lib = include_bytes!(env!("XBERG_PDFIUM_BUNDLED_PATH"));
...
mod tests {
    ...
    fn test_get_extraction_dir() {
        ...
        assert!(dir.ends_with("xberg-pdfium"));
    }
