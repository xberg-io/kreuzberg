...
# alef:hash:3979aa7979589226d0fe8824d307e7b9d684bf08a8f8f8608b916202c11320dc
...
async def extract_file(
    ...
    if _rust_config is None:
        msg = "config conversion returned None"
        raise ValueError(msg)
    return await _rust.extract_file(path=path, config=_rust_config, mime_type=mime_type)
...
def extract_file_sync(
    ...
    if _rust_config is None:
        msg = "config conversion returned None"
        raise ValueError(msg)
    return _rust.extract_file_sync(path=path, config=_rust_config, mime_type=mime_type)
...
def chunk_text_with_heading_source(
    ...
    if _rust_config is None:
        msg = "config conversion returned None"
        raise ValueError(msg)
    return _rust.chunk_text_with_heading_source(text=text, config=_rust_config, page_boundaries=page_boundaries, heading_source=heading_source)
...
def calculate_optimal_dpi(
    ...
    return _rust.calculate_optimal_dpi(page_width=page_width, page_height=page_height, target_dpi=target_dpi, max_dimension=max_dimension, min_dpi=min_dpi, max_dpi=max_dpi)
