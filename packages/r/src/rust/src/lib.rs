...
// alef:hash:6bdde0767753a89082a122d495afca967e967af46e838d0fc5fefaf585fee763
...
pub fn extract_file_sync(path: String, mime_type: Option<String>, config: Option<ExtractionConfig>) -> Result<ExtractionResult> {
    let config_json = serde_json::to_string(&config).map_err(|e| e.to_string())?;
    let config_core: kreuzberg::ExtractionConfig = serde_json::from_str(&config_json).map_err(|e| e.to_string())?;
    kreuzberg::extract_file_sync(path, mime_type.as_deref(), &config_core).map(ExtractionResult::from).map_err(|e| e.to_string())
}

#[allow(clippy::missing_errors_doc)]
#[extendr]
pub fn extract_file_sync_ffi(path: String, mime_type: String, config: ExtractionConfig) -> Result<ExtractionResult> {
    let config_json = serde_json::to_string(&config).map_err(|e| e.to_string())?;
    let config_core: kreuzberg::ExtractionConfig = serde_json::from_str(&config_json).map_err(|e| e.to_string())?;
    kreuzberg::extract_file_sync_ffi(&path, &mime_type, &config_core).map(ExtractionResult::from).map_err(|e| e.to_string())
}

#[allow(clippy::missing_errors_doc)]
#[extendr]
pub async fn extract_file_ffi(path: String, mime_type: String, config: ExtractionConfig) -> Result<ExtractionResult> {
    let config_json = serde_json::to_string(&config).map_err(|e| e.to_string())?;
    let config_core: kreuzberg::ExtractionConfig = serde_json::from_str(&config_json).map_err(|e| e.to_string())?;
    todo!("async not supported by backend")
}
