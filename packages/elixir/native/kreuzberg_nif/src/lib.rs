...
// alef:hash:acafd27f5e4988fa7ba9fe873b5a4e475f7521819ecfe8be4cef52310139337b
...
pub fn extract_file_sync_ffi(
    ...
    let config_core: Option<kreuzberg::ExtractionConfig> = config
        ...
        .map_err(|e| e.to_string())?;
    let result =
        kreuzberg::extract_file_sync_ffi(&path, &mime_type, config_core.as_ref().unwrap_or(&Default::default()))
            .map_err(|e| e.to_string())?;
    Ok(result.into())
}

#[rustler::nif(schedule = "DirtyCpu")]
pub fn extract_file_ffi_async(
    path: String,
    mime_type: String,
    config: Option<String>,
) -> Result<ExtractionResult, String> {
    ...
    let result = rt
        .block_on(async {
            kreuzberg::extract_file_ffi(&path, &mime_type, config_core.as_ref().unwrap_or(&Default::default())).await
        })
        .map_err(|e| e.to_string())?;
    Ok(result.into())
}
