...
pub fn analyze_document(metadata: DocumentMetadata, config: HeuristicsConfig, document_bytes: ?[]const u8) KreuzbergError!ChunkingDecision {
    const _result = c.kreuzberg_analyze_document(metadata, config, document_bytes.ptr, document_bytes.len);
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
...
pub fn analyze_with_user_chunks(user_ranges: []const u8, total_pages: u32, size_bytes: u64, config: HeuristicsConfig) error{OutOfMemory}!ChunkingDecision {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    ...
    const _result = c.kreuzberg_analyze_with_user_chunks(user_ranges_z, total_pages, size_bytes, config);
    return ChunkingDecision{ ._handle = _result.? };
}
...
pub fn score_confidence(signals: ConfidenceSignals, weights: ConfidenceWeights) error{OutOfMemory}![]u8 {
    const _result = c.kreuzberg_score_confidence(signals, weights);
    return blk: {
...
pub fn check_format_limits(mime_type: []const u8, sheet_count: ?u32, workbook_cells: ?u64, embedded_count: ?u32, config: HeuristicsConfig) error{OutOfMemory}!?[]u8 {
    const mime_type_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{mime_type}, 0);
    defer std.heap.c_allocator.free(mime_type_z);
    const _result = c.kreuzberg_check_format_limits(mime_type_z, if (sheet_count) |v| v else std.math.maxInt(u32), if (workbook_cells) |v| v else std.math.maxInt(u64), if (embedded_count) |v| v else std.math.maxInt(u32), config);
    const _result_len = c.kreuzberg_check_format_limits_len(mime_type_z, if (sheet_count) |v| v else std.math.maxInt(u32), if (workbook_cells) |v| v else std.math.maxInt(u64), if (embedded_count) |v| v else std.math.maxInt(u32), config);
    return blk: {
        if (_result == null) break :blk null;
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
///   `Skip`).
pub fn choose_call_mode(input: StructuredInput, t: StructuredThresholds) error{OutOfMemory}!StructuredCallMode {
    const _result = c.kreuzberg_choose_call_mode(input, t);
...
pub fn calculate_chunk_plan(page_count: u32, size_bytes: u64, needs_ocr: bool, config: HeuristicsConfig) error{OutOfMemory}!ChunkPlan {
    const _result = c.kreuzberg_calculate_chunk_plan(page_count, size_bytes, needs_ocr, config);
    return ChunkPlan{ ._handle = _result.? };
}

/// Calculate a chunk plan from user-specified page ranges.
///
/// Validates and processes user overrides into a proper chunk plan.
pub fn calculate_plan_from_overrides(user_chunks: []const u8, total_pages: u32, size_bytes: u64, config: HeuristicsConfig) error{OutOfMemory}!ChunkPlan {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    ...
    const _result = c.kreuzberg_calculate_plan_from_overrides(user_chunks_z, total_pages, size_bytes, config);
    return ChunkPlan{ ._handle = _result.? };
}
...
pub fn resolve(preset: []const u8, custom_schema: ?[]const u8, context: []const u8) ResolveError!ResolvedPreset {
    ...
    return ResolvedPreset{ ._handle = _result.? };
}
...
pub const DocumentMetadata = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *DocumentMetadata) void {
        c.kreuzberg_document_metadata_free(@as(*c.KREUZBERGDocumentMetadata, @ptrCast(self._handle)));
    }
};
...
pub const UserChunkConfig = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *UserChunkConfig) void {
        c.kreuzberg_user_chunk_config_free(@as(*c.KREUZBERGUserChunkConfig, @ptrCast(self._handle)));
    }
};
...
pub fn from_extraction_result_confidence_signals(result: []const u8, schema_compliance: SchemaCompliance, text_coverage: f32) error{OutOfMemory,InvalidJson}!ConfidenceSignals {
    const result_z = try std.heap.c_allocator.dupeZ(u8, result);
    defer std.heap.c_allocator.free(result_z);
    const result_handle = c.kreuzberg_extraction_result_from_json(result_z.ptr);
    if (result_handle == null) return error.InvalidJson;
    defer c.kreuzberg_extraction_result_free(result_handle);
    const schema_compliance_i32: i32 = @intFromEnum(schema_compliance);
    const _handle = c.kreuzberg_confidence_signals_from_extraction_result(result_handle, schema_compliance_i32, text_coverage);
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.KREUZBERGConfidenceSignals, @ptrCast(_handle.?)) };
}
...
pub const ConfidenceSignals = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ConfidenceSignals) void {
        c.kreuzberg_confidence_signals_free(@as(*c.KREUZBERGConfidenceSignals, @ptrCast(self._handle)));
    }
};

pub fn default_confidence_weights() ConfidenceWeights {
    const _handle = c.kreuzberg_confidence_weights_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.KREUZBERGConfidenceWeights, @ptrCast(_handle.?)) };
}
...
pub const ConfidenceWeights = struct {
    ...
    pub fn is_normalized(self: *ConfidenceWeights) bool {
        const _result = c.kreuzberg_confidence_weights_is_normalized(@as(*c.KREUZBERGConfidenceWeights, @ptrCast(self._handle)));
        return _result;
    }

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ConfidenceWeights) void {
        c.kreuzberg_confidence_weights_free(@as(*c.KREUZBERGConfidenceWeights, @ptrCast(self._handle)));
    }
};

pub fn default_heuristics_config() HeuristicsConfig {
    const _handle = c.kreuzberg_heuristics_config_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.KREUZBERGHeuristicsConfig, @ptrCast(_handle.?)) };
}

/// Create a configuration suitable for unit tests (smaller thresholds).
pub fn test_config_heuristics_config() HeuristicsConfig {
    const _handle = c.kreuzberg_heuristics_config_test_config();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.KREUZBERGHeuristicsConfig, @ptrCast(_handle.?)) };
}
...
pub const HeuristicsConfig = struct {
    ...
    pub fn validate(self: *HeuristicsConfig) (KreuzbergError||error{OutOfMemory})!void {
        _ = c.kreuzberg_heuristics_config_validate(@as(*c.KREUZBERGHeuristicsConfig, @ptrCast(self._handle)));
        if (c.kreuzberg_last_error_code() != 0) {
            return _error_with_message(KreuzbergError);
        }
    }
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *HeuristicsConfig) void {
        c.kreuzberg_heuristics_config_free(@as(*c.KREUZBERGHeuristicsConfig, @ptrCast(self._handle)));
    }
};
...
pub const ChunkPlan = struct {
    ...
    pub fn total_pages(self: *ChunkPlan) u32 {
        const _result = c.kreuzberg_chunk_plan_total_pages(@as(*c.KREUZBERGChunkPlan, @ptrCast(self._handle)));
        return _result;
    }

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ChunkPlan) void {
        c.kreuzberg_chunk_plan_free(@as(*c.KREUZBERGChunkPlan, @ptrCast(self._handle)));
    }
};
...
pub const StructuredInput = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *StructuredInput) void {
        c.kreuzberg_structured_input_free(@as(*c.KREUZBERGStructuredInput, @ptrCast(self._handle)));
    }
};

pub fn default_structured_thresholds() StructuredThresholds {
    const _handle = c.kreuzberg_structured_thresholds_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.KREUZBERGStructuredThresholds, @ptrCast(_handle.?)) };
}
...
pub const StructuredThresholds = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *StructuredThresholds) void {
        c.kreuzberg_structured_thresholds_free(@as(*c.KREUZBERGStructuredThresholds, @ptrCast(self._handle)));
    }
};
...
pub const ResolvedPreset = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ResolvedPreset) void {
        c.kreuzberg_resolved_preset_free(@as(*c.KREUZBERGResolvedPreset, @ptrCast(self._handle)));
    }
};
