...
pub const DocExtractionResult = struct {
    content: [:0]const u8,
...
pub const DocMetadata = struct {
    ...
    subject: ?[:0]const u8,
    author: ?[:0]const u8,
    last_author: ?[:0]const u8,
    created: ?[:0]const u8,
    modified: ?[:0]const u8,
    revision_number: ?[:0]const u8,
};
...
pub fn validate_port(port: u32) (KreuzbergError || error{OutOfMemory})!void {
...
pub fn render_html_str(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_render_html_str(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
    return blk: {
        const slice = std.mem.sliceTo(_result, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
///
/// Returns an error if the HTML cannot be parsed.
pub fn render_markdown_str(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_render_markdown_str(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
...
///
/// Returns an error if the HTML cannot be parsed.
pub fn render_djot_str(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_render_djot_str(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
...
///
/// Returns an error if the HTML cannot be parsed.
pub fn render_json_str(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_render_json_str(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
...
///
/// Returns an error if the HTML cannot be parsed.
pub fn render_plain_str(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_render_plain_str(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
...
pub fn detect_image_format(data: []const u8) []u8 {
    const _result = c.kreuzberg_detect_image_format(data.ptr, data.len);
    return blk: {
        const slice = std.mem.sliceTo(_result, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
pub fn generate_cache_key(parts: []const u8) []u8 {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const parts_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{parts},
    );
    std.heap.c_allocator.free(parts_z[0..std.mem.len(parts_z)]);
    const _result = c.kreuzberg_generate_cache_key(parts_z);
    return blk: {
        const slice = std.mem.sliceTo(_result, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
pub fn escape_html_entities(text: []const u8) []u8 {
    const text_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{text},
    ...
    std.heap.c_allocator.free(text_z[0..std.mem.len(text_z)]);
    const _result = c.kreuzberg_escape_html_entities(text_z);
    return blk: {
        const slice = std.mem.sliceTo(_result, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
pub fn fix_mojibake(text: []const u8) []u8 {
    const text_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{text},
    ...
    std.heap.c_allocator.free(text_z[0..std.mem.len(text_z)]);
    const _result = c.kreuzberg_fix_mojibake(text_z);
    return blk: {
        const slice = std.mem.sliceTo(_result, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
pub fn convert_html_to_markdown(html: []const u8) (KreuzbergError || error{OutOfMemory})![]u8 {
    const html_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{html},
    );
    const _result = c.kreuzberg_convert_html_to_markdown(html_z);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(html_z[0..std.mem.len(html_z)]);
...
pub fn extract_doc_text(content: []const u8) (KreuzbergError || error{OutOfMemory})!DocExtractionResult {
    const _result = c.kreuzberg_extract_doc_text(content.ptr, content.len);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    return _result;
...
pub fn extract_pptx_from_bytes(data: []const u8) (KreuzbergError || error{OutOfMemory})!PptxExtractionResult {
    const _result = c.kreuzberg_extract_pptx_from_bytes(data.ptr, data.len);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    return _result;
...
pub fn extract_file_to_toon(path: []const u8, config: ExtractionConfig) (KreuzbergError || error{OutOfMemory})![]u8 {
    const path_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{path},
    );
    const _result = c.kreuzberg_extract_file_to_toon(path_z, config);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(path_z[0..std.mem.len(path_z)]);
...
pub fn extract_file_to_json(path: []const u8, config: ExtractionConfig) (KreuzbergError || error{OutOfMemory})![]u8 {
    const path_z: [*:0]u8 = try std.fmt.allocPrintZ(
        std.heap.c_allocator,
        "{s}",
        .{path},
    );
    const _result = c.kreuzberg_extract_file_to_json(path_z, config);
    if (c.kreuzberg_last_error_code() != 0) {
        return _first_error(KreuzbergError);
    }
    std.heap.c_allocator.free(path_z[0..std.mem.len(path_z)]);
