...
object Kreuzberg {
     ...
     * Since v5.0.0.
     */
    suspend fun rerankAsync(query: String, documents: List<String>, config: RerankerConfig): List<RerankedDocument> =
        withContext(Dispatchers.IO) { rerank(query, documents, config) }
    /**
    ...
    fun rerankAsync(query: String, documents: List<String>, config: RerankerConfig): List<RerankedDocument> {
        val resultJson = KreuzbergBridge.nativeRerankAsync(query, mapper.writeValueAsString(documents), mapper.writeValueAsString(config))
        return mapper.readValue(resultJson, object : TypeReference<List<RerankedDocument>>() {})
    }
    /**
     * Stub for builds without the `reranker` feature.
     ...
     * Since v5.0.0.
     */
    suspend fun rerankAsyncAsync(query: String, documents: List<String>, config: RerankerConfig): List<RerankedDocument> =
        withContext(Dispatchers.IO) { rerankAsync(query, documents, config) }
    /**
