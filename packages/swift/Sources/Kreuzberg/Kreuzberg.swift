...
public typealias UserChunkConfig = RustBridge.UserChunkConfig
...
public typealias ExtractionConfidence = RustBridge.ExtractionConfidence
...
public typealias StructuredInput = RustBridge.StructuredInput
...
public func scoreConfidence(signals: ConfidenceSignals, weights: ConfidenceWeights) -> ExtractionConfidence {
    return RustBridge.scoreConfidence(signals, weights)
}
...
public func checkFormatLimits(mimeType: String, sheetCount: UInt32? = nil, workbookCells: UInt64? = nil, embeddedCount: UInt32? = nil, config: HeuristicsConfig) -> String? {
        let _rb_mimeType = RustString(mimeType)
    let _rb_json = RustBridge.checkFormatLimits(_rb_mimeType, sheetCount, workbookCells, embeddedCount, config).toString()
    let _rb_data = _rb_json.data(using: .utf8) ?? Data()
    return (try? JSONDecoder().decode(String?.self, from: _rb_data)) ?? nil
}
...
public func chooseCallMode(input: StructuredInput, t: StructuredThresholds) throws -> StructuredCallMode {
    let _rb = RustBridge.chooseCallMode(input, t)
...
public func calculateChunkPlan(pageCount: UInt32, sizeBytes: UInt64, needsOcr: Bool, config: HeuristicsConfig) -> ChunkPlan {
    return RustBridge.calculateChunkPlan(pageCount, sizeBytes, needsOcr, config)
}
...
public func calculatePlanFromOverrides(userChunks: [PageRange], totalPages: UInt32, sizeBytes: UInt64, config: HeuristicsConfig) throws -> ChunkPlan {
    ...
    return RustBridge.calculatePlanFromOverrides(_rb_userChunks, totalPages, sizeBytes, config)
}
