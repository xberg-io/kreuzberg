...
=== "TypeScript"
    ...
    ```typescript
    import { ExtractionConfig } from 'xberg';
...
=== "Go"
    ...
    ```go
    import "github.com/xberg-io/xberg/v4"
...
=== "Rust"
    ...
    ```rust
    use xberg::{ExtractionConfig, PdfConfig, FontConfig};
...
=== "Python"
    ...
    ```python
    from xberg import ExtractionConfig, PdfConfig, FontConfig
...
=== "Ruby"
    ...
    config = Xberg::ExtractionConfig.new(
      pdf_options: Xberg::PdfConfig.new(
        font_config: Xberg::FontConfig.new(enabled: false)
      )
    )
    ```
...
=== "Ruby"
    ...
    config = Xberg::ExtractionConfig.new(
      pdf_options: Xberg::PdfConfig.new(
        font_config: Xberg::FontConfig.new(
          enabled: true,
          custom_font_dirs: [
            '/usr/share/fonts/custom',
            '~/my-fonts'  # Tilde expanded automatically
...
=== "Rust"
    ...
    let result = xberg::extract_file("document.pdf", &config)?;
    ...
    let result2 = xberg::extract_file("document2.pdf", &new_config)?;
...
=== "Java"
    ...
    ExtractionConfig newConfig = ExtractionConfig.builder()
        ...
        .build();
    ExtractionResult result2 = Xberg.extractFile("document2.pdf", newConfig);
    // Warning logged: "Font config already initialized"
    ```
...
=== "Go"
    ...
    result, _ := xberg.ExtractFile("document.pdf", config)

    // INCORRECT: Attempting to change config after first extraction
    newConfig := &xberg.ExtractionConfig{
        PdfOptions: &xberg.PdfConfig{
            FontConfig: &xberg.FontConfig{
                Enabled: false,
            },
    ...
    result2, _ := xberg.ExtractFile("document2.pdf", newConfig)
    // Warning logged: "Font config already initialized"
    ```
...
=== "Ruby"
    ...
    result = Xberg.extract_file('document.pdf', config)

    # INCORRECT: Attempting to change config after first extraction
    new_config = Xberg::ExtractionConfig.new(
      pdf_options: Xberg::PdfConfig.new(
        font_config: Xberg::FontConfig.new(enabled: false)
      )
    )
    result2 = Xberg.extract_file('document2.pdf', new_config)
    # Warning logged: "Font config already initialized"
    ```
...
=== "C#"
    ...
    var pdfConfig = new PdfConfig { FontConfig = fontConfig };
    var config = new ExtractionConfig { PdfOptions = pdfConfig };
    var result = Xberg.ExtractFile("document.pdf", config);
    ...
    var result2 = Xberg.ExtractFile("document2.pdf", newConfig);
    // Warning logged: "Font config already initialized"
    ```
...
- **Issue tracker**: <https://github.com/xberg-io/xberg/issues>
